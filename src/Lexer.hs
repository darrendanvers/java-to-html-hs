...
toTokenType t 
    ...
    | t == "]" = RightBracket
    | t == "," = Comma
    | t == [] = End               -- A quirk I couldn't work around was an empty string after the end of the file. 
    | otherwise = Character
...
isDelimiter c = elem (toTokenType c) delimiters
...
tillNotReserved (x:xs)
    | isSpace x = tillNotReserved xs       -- Ignore whitespace.
    | isDelimiter [x] = ([x], xs)          -- If the frist character is a delimiter, return it by itself.
    | xs == [] = ([x], xs)                 -- If the remainder of the string is empty, just retunrn the head.
    ...
    | otherwise = (x : y, z) where         -- For everything else, return the first character appended
        (y, z) = tillNotReserved xs        -- to the restlt of calling this function again with the remainder of the string.
